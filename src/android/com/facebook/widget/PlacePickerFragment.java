...
public class PlacePickerFragment extends PickerFragment<GraphPlace> {
    ...
    public PlacePickerFragment(Bundle args) {
        super(GraphPlace.class, IntelXDKFacebook.getId("layout", "com_facebook_placepickerfragment"), args);
    ...
    public void onInflate(Activity activity, AttributeSet attrs, Bundle savedInstanceState) {
        ...
        TypedArray a = activity.obtainStyledAttributes(attrs, IntelXDKFacebook.getStyleableIntArray("com_facebook_place_picker_fragment"));

        setRadiusInMeters(a.getInt(IntelXDKFacebook.getId("styleable", "com_facebook_place_picker_fragment_radius_in_meters"), radiusInMeters));
        setResultsLimit(a.getInt(IntelXDKFacebook.getId("styleable", "com_facebook_place_picker_fragment_results_limit"), resultsLimit));
        if (a.hasValue(IntelXDKFacebook.getId("styleable", "com_facebook_place_picker_fragment_results_limit"))) {
            setSearchText(a.getString(IntelXDKFacebook.getId("styleable", "com_facebook_place_picker_fragment_search_text")));
        }
        showSearchBox = a.getBoolean(IntelXDKFacebook.getId("styleable", "com_facebook_place_picker_fragment_show_search_box"), showSearchBox);
    ...
    void setupViews(ViewGroup view) {
        if (showSearchBox) {
            ListView listView = (ListView) view.findViewById(IntelXDKFacebook.getId("id", "com_facebook_picker_list_view"));

            View searchHeaderView = getActivity().getLayoutInflater().inflate(
            		IntelXDKFacebook.getId("layout", "com_facebook_picker_search_box"), listView, false);

            listView.addHeaderView(searchHeaderView, null, false);

            searchBox = (EditText) view.findViewById(IntelXDKFacebook.getId("id", "com_facebook_picker_search_text"));
    ...
    String getDefaultTitleText() {
        return getString(IntelXDKFacebook.getId("string", "com_facebook_nearby"));
    }
        ...
        PickerFragmentAdapter<GraphPlace> adapter = new PickerFragmentAdapter<GraphPlace>(
            ...
            protected CharSequence getSubTitleOfGraphObject(GraphPlace graphObject) {
                ...
                if (category != null && wereHereCount != null) {
                    result = getString(IntelXDKFacebook.getId("string", "com_facebook_placepicker_subtitle_format"), category, wereHereCount);
                } else if (category == null && wereHereCount != null) {
                    result = getString(IntelXDKFacebook.getId("string", "com_facebook_placepicker_subtitle_were_here_only_format"), wereHereCount);
                } else if (category != null && wereHereCount == null) {
                    result = getString(IntelXDKFacebook.getId("string", "com_facebook_placepicker_subtitle_catetory_only_format"), category);
                }
                return result;
            }
            ...
            protected int getGraphObjectRowLayoutId(GraphPlace graphObject) {
                return IntelXDKFacebook.getId("layout", "com_facebook_placepickerfragment_list_row");
            }

            @Override
            protected int getDefaultPicture() {
                return IntelXDKFacebook.getId("drawable", "com_facebook_place_default_icon");
            }
